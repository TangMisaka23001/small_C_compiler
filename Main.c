...
int *text,     //代码段
    ...
    *stack;    //栈
char *data;    //数据段
// 寄存器使用变量:PC程序计数器 SP指针寄存器 BP基址指针 AX通用寄存器
...
void next()
{
    ...
    else if ((token >= 'a' && token <= 'z') || (token >= 'A' && token <= 'Z') || (token == '_')) //分析标识符
    {
        //parse identifiter
        last_pos = src - 1;
        hash = token;
...
void function_parameter()
{
    ...
    while (token != ')')
    {
        ...
        if (current_id[Class] == Loc)
        {
            printf("%d: duplocate parameter declaration\n", line);
            exit(-1);
        }
        match(Id);
...
void enum_declaration()
{
    ...
    while (token != '}')
    {
        if (token != Id)
        {
            printf("%d: bad enum identifiter %d\n", line, token);
            exit(-1);
        }
        next();
        if (token == Assign)
...
int eval()
{
    ...
    {
        op = *pc++;
        if (op == IMM) //将num放入寄存器中
        {
            ax = *pc++;
        }
        else if (op == LC) //将对应地址字符载入ax中
        {
            ax = *(char *)ax;
        }
        else if (op == LI) //将对应地址整数载入ax中
        {
            ax = *(int *)ax;
        }
        else if (op == SC) //将ax中的数据作为字符放入地址中
        {
            ax = *(char *)*sp++ = ax;
        }
        else if (op == SI) //将ax中的数据作为整数放入地址中
        {
            *(int *)*sp++ = ax;
        }
        else if (op == PUSH) //将ax的值放入栈中
        {
            *--sp = ax;
        }
        else if (op == JMP) //跳转指令
        ...
        */
        ...
        else if (op == LEA) //load address for arg
        {
            ax = (int)(bp + *pc++);
        }
        //接下来是运算符指令
